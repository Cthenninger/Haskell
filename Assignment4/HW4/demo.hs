...
test (Not t) w r = not(test t w  r) -- flip boolean
test (Facing ca) w r = ca == getFacing r -- helper function from KarelState.hs
test (Clear d) w r = isClear (relativePos d r) w  -- change next step to Pos and pass that to clear
test (Beeper) w r = hasBeeper (getPos r) w  -- get current position and check for beeper
...
stmt Move       _ w r = let p = relativePos Front r -- assign p the position directly infront of r
                                    in if isClear p w
...
stmt PickBeeper  _ w r           = let p = getPos r
                                      in if hasBeeper p w
                                          ...
                                          else Error ("No beeper to pick: " ++ show p)
...
stmt (Iterate num eval) de w r = if num /= 0 then
                                            case stmt eval de w r of
                                              (OK w' r') -> stmt (Iterate (num - 1) eval) de w' r'
                                              (Done r') -> Done r'
                                              (Error m) -> Error m
                                           else Error ("Fail in the loop, but why? =P")
stmt (If tr pa fa) de w r = if (test tr w r == True)
                                                then stmt pa de w r
                                                else stmt fa de w r
